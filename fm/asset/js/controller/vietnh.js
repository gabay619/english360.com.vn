app.controller('ExplorerCtrl3', function ($scope, $controller) {
    $controller('ExplorerCtrl2', { $scope: $scope });
    // All out code here!
    $scope.showcontext = function (o) {
        $scope.item = o;
        $(function () {
            $(".mylistfile .myfileitem").contextmenu({
                target: '#context-menu2',
                onItem: function (e, item) {
                    var action = item.attr('data-value');
                    if (action == 'View' || action == 'Rename') {
                        $scope.fileinfo($scope.item);
                        $('#fileModal').modal('show');
                    }
                    else if (action == 'Delete') {
                        $scope.delfile(o.filename);
                    }
                }
            });
        });
