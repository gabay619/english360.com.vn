...
//    }
    public static function maketypegtcb($name,$cid)
    {
        global $domain;
        $url = makelink::Cleanurl(makelink::khongdau($name));
        return $domain.'/'.$url.'-tga'.$cid.'zv.html';
    }
    ...
    static function makegtcb($id,$name){
        global $domain;
        $url = makelink::Cleanurl(makelink::khongdau($name));
        return $domain.'/giao-tiep-co-ban/'.$url.'-'.$id.'.html';
    }
    static function makelna($id,$name){
        global $domain;
        $url = makelink::Cleanurl(makelink::khongdau($name));
        return $domain.'/luyen-ngu-am/'.$url.'-'.$id.'.html';
    }
    static function makehmcaudio($id,$name){
        global $domain;
        $url = makelink::Cleanurl(makelink::khongdau($name));
        return $domain.'/bai-hat/'.$url.'-'.$id.'.html';
    }
...
//        return "/hmcaudio.php?catid=$id";
        global $domain;
        $url = makelink::Cleanurl(makelink::khongdau($name));
        return $domain.'/bai-hat-tieng-anh/'.$url.'-'.$id.'.html';
    }
//    static function makehmcaudio($id,$name){
...
//    }
...

    {
        ...
        $text = str_replace('-',' ',$text);$text = str_replace(' -',' ',$text);$text = str_replace(array('&apos;','&quot;'),'',$text);

        $text = preg_replace('/[^a-zA-Z0-9_ -,.]/s','', $text);$text = trim($text);

        $stripped = preg_replace(array('/\s{2,}/', '/[\t\n]/'), ' ', $text);

        $text=strtolower($stripped);$text = str_replace(',',' ',$text);
