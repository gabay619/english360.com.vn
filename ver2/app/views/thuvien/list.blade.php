    ...
    <div class="content_left">
        <div class="block">
            <div class="heading_cate">
                <h2><strong>{{$parentCate->name}}</strong><label class="label_heading_cate"></label></h2>
                <ul class="list_heading_cate">
                    @foreach($cateChild as $aCate)
                        <li><a href="{{ThuVien::getCateUrl($aCate->_id, $aCate->name, $type)}}" title="">{{$aCate->name}}</a></li>
                    @endforeach
                </ul>
            </div>
