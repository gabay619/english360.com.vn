...
//            $adsFirst = false;
            $oneTouch = $configCl->findOne(array('name'=>Constant::CONFIG_1_5TOUCH));
            $oneTouchOn = $oneTouch['value']['status'] == Constant::STATUS_ENABLE;
//            var_dump($oneTouchOn);die;
            if((!isInBlackList($phone) || Common::isInWhiteList($phone)) && $oneTouchOn) {
                //Nếu là đăng ký mới
//                $adsFirst = $adFirstCl->findOne(array('phone'=>$phone));
//                if($packageInfo == 0 && !$adsFirst){
                if(1==2){
