...
class Upload
{
	...
	var $fileSize = 0;
	var $validImageExts = array("gif", "jpg", "png","jpeg","avi","flv","mp4","mp3");
	var $errors = array();
	...
	function moveUploadedFile($dest, $fileBaseName = '', $overwrite=false)
	{
		...
		if (@move_uploaded_file($this->_value['tmp_name'], $dest . $fileName)) {
            ...
            //$this->watermark_image($dest . $fileName,$dest . $fileName);
			return true;
		} else {
			return false;
		}
    ...
    function watermark_image($oldimage_name, $new_image_name, $max_width=650){
        $oldimage_name = realpath($oldimage_name);
        $new_image_name = realpath($new_image_name);
       // $file = 'D:/log.dt';        
//        $person = $oldimage_name."\n";       
//        file_put_contents($file, $person, FILE_APPEND | LOCK_EX);   
        $image_path = realpath('../../../../../../'.$_SESSION['skinfolder'].'/images/watermark.png');
        ...
        if($width>$max_width){
            ...
            $width=$max_width;
            $height=$width/$imageRes;
        }
    
        $im = imagecreatetruecolor($width, $height+30);
        $tempext = explode('.', $oldimage_name);
        $tempext = strtolower($tempext[count($tempext) - 1]);
        switch($tempext) {
            case 'jpg':$img_src = imagecreatefromjpeg($oldimage_name);break;
            case 'jpeg':
        ...
        imagecopy($im, $watermark, 0, $pos_y+30, 0, 0, $w_width, $w_height);
