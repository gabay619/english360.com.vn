...
*/
$upfolder = '../../../../../uploads/' . md5($_SESSION['username']) . '/';
if (!file_exists($upfolder)) mkdir($upfolder, 0777, true);
$difile = $upfolder .  "_n.data";
if (!file_exists($difile)) {
    $fp = fopen($difile, "wb");
    fwrite($fp, $_SESSION['username']);
    fclose($fp);
}
define('CONFIG_SYS_DEFAULT_PATH', $upfolder); //accept relative path only
define('CONFIG_SYS_ROOT_PATH', $upfolder); //accept relative path only
define('CONFIG_SYS_FOLDER_SHOWN_ON_TOP', true); //show your folders on the top of list if true or order by name
//define("CONFIG_SYS_DIR_SESSION_PATH", '/admincp/js/plugins/tinymce/plugins/ajaxfilemanager/session/');
if (!file_exists($_SERVER['DOCUMENT_ROOT'] . "/session/")) mkdir($_SERVER['DOCUMENT_ROOT'] . "/session/", 0777, true);
define("CONFIG_SYS_DIR_SESSION_PATH", $_SERVER['DOCUMENT_ROOT'] . '/session/');
...
define('CONFIG_UPLOAD_MAXSIZE', 256 * 1024 * 1024); //by bytes
...
//define('CONFIG_UPLOAD_VALID_EXTS', 'gif,jpg,png,txt,flv,mov,mp4,avi,mp3,wav,oog'); //
...
define('CONFIG_UPLOAD_INVALID_EXTS', 'php,aspx,asp,exe');
//Preview
