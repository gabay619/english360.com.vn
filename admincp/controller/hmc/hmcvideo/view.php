...
<?php if(acceptpermiss("hmcvideo_insert")) { ?><a href="<?php echo cpagerparm("tact,id,status") ?>tact=hmcvideo_insert">Thêm mới</a><?php }?>
</div>
...
<table class="table table-hover">
    ...
    <?php foreach ($listnews as $item) { ?>
        <tr>
            ...
            <td>
                <?php if(acceptpermiss("hmcvideo_update")) { ?><a href="<?php echo cpagerparm("tact,status,id") ?>tact=hmcvideo_update&id=<?php echo $item['_id'] ?>">Sửa</a> |<?php }?>
                <?php if(acceptpermiss("hmcvideo_delete")) { ?><a onclick="return confirm('Bạn chắc chắn chứ?')" href="<?php echo cpagerparm("tact,status,id") ?>tact=hmcvideo_delete&id=<?php echo $item['_id'] ?>">Xóa</a><?php }?>
            </td>
        </tr>
    <?php } ?>
    </tbody>
