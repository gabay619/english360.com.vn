...
if(isset($q)){
    $cond = array(
        '$or'=>array(
                array('namenonutf'=>new MongoRegex("/$q/ui")),
                array('_id'=>"$q")
        )
    );
}
...
<?php if(acceptpermiss("hmcaudio_insert")) { ?><a href="<?php echo cpagerparm("tact,id,status") ?>tact=hmcaudio_insert">Thêm mới</a><?php }?>
</div>
<div class="col-xs-12">
...
<table class="table table-hover">
    ...
    <?php foreach ($listnews as $item) {
        ...
        <tr>
            ...
            <td><a href="<?php echo Constant::BASE_URL.'/bai-hat/'.Common::utf8_to_url($item['name']).'-'.$item['_id'].'.html' ?>" target="_blank"><?php echo $item['name'] ?></a>
            ...
            <td>
                <?php if(acceptpermiss("hmcaudio_update")) { ?><a href="<?php echo cpagerparm("tact,status,id") ?>tact=hmcaudio_update&id=<?php echo $item['_id'] ?>">Sửa</a> |<?php }?>
                <?php if(acceptpermiss("hmcaudio_update")) { ?><a onclick="return confirm('Bạn chắc chắn chứ?')" href="<?php echo cpagerparm("tact,status,id") ?>tact=hmcaudio_delete&id=<?php echo $item['_id'] ?>">Xóa</a> | <?php }?>
                <?php if(acceptpermiss("audio_upload_view")) { ?><a href="<?php echo cpagerparm("tact,status,id") ?>tact=audio_upload_view&hmcaudioid=<?php echo $item['_id'] ?>">Audio User Upload</a><?php }?>
            </td>
        </tr>
    <?php } ?>
    </tbody>
    ...
    <script>
        function changeStatus (atid, status) {
            ...
            $.post('incoming.php',{act: 'changestatusaudio', atid: atid, status: status, isUpdateTime: isUpdateTime}, function(res) {
                if (res.status == 200) {
        ...
        function changeFree (atid, status) {
            $.post('incoming.php',{act: 'changefree', atid: atid, status: status, type: '<?php echo Constant::TYPE_SONG ?>'}, function(res) {
                if (res.status == 200) {
