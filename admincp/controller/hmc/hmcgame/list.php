...
$listcat = iterator_to_array($categorycl->find(array("type"=>"hocmachoi"))->sort($sort));
?>
    ...
    <table class="table table-hover text-left">
        ...
        <?php foreach ($listcat as $cat) { ?>
            <tr>
                ...
                <td><a href="<?php echo cpagerparm("act,tact,status,id") ?>act=<?php echo "hmcgame"; ?>&tact=hmcgame_view&catid=<?php echo $cat['_id'] ?>"><?php echo $cat['name'] ?></a></td>
                <td><?php echo date("d-m-Y H:i:s", $cat['_id']) ?></td>
                <td>
                    <?php if(acceptpermiss("hmcgame_view")) { ?><a href="<?php echo cpagerparm("act,tact,status,id") ?>act=hmcgame&tact=hmcgame_view&catid=<?php echo $cat['_id'] ?>">Xem bài đăng</a><?php } ?>
                </td>
            </tr>
        <?php } ?>
        </tbody>
