<?php
$title = "Quản lý bài luyện ngữ âm";
$categorycl = $dbmg->category;
$newscl = $dbmg->luyennguam;
$usercl = $dbmg->user;
#condition
$limit = 25;
...
<?php if(acceptpermiss("luyennguam_insert")) { ?><a href="<?php echo cpagerparm("tact,id,status") ?>tact=luyennguam_insert">Thêm mới</a><?php }?>
</div>
<div class="col-xs-12">
...
/*                    if(count($item['category'])<=0) $item['category'] = array();
            ...
            <td>
                <?php if(acceptpermiss("luyennguam_test_view")) { ?><a href="<?php echo cpagerparm("tact,status,id,p") ?>tact=lna_tn_view&lnaid=<?php echo $item['_id'] ?>">BT trắc nghiệm</a> |<?php } ?>
                <?php if(acceptpermiss("luyennguam_test_view")) { ?><a href="<?php echo cpagerparm("tact,status,id,p") ?>tact=lna_dc_view&lnaid=<?php echo $item['_id'] ?>">BT điền chữ</a> |<?php } ?>
                <?php if(acceptpermiss("luyennguam_test_view")) { ?><a href="<?php echo cpagerparm("tact,status,id,p") ?>tact=lna_dt_view&lnaid=<?php echo $item['_id'] ?>">BT điền từ</a> |<?php } ?>
                <?php if(acceptpermiss("luyennguam_test_view")) { ?><a href="<?php echo cpagerparm("tact,status,id,p") ?>tact=lna_pa_view&lnaid=<?php echo $item['_id'] ?>">BT phát âm</a> |<?php } ?>
                <?php if(acceptpermiss("luyennguam_test_view")) { ?><a href="<?php echo cpagerparm("tact,status,id,p") ?>tact=lna_xt_view&lnaid=<?php echo $item['_id'] ?>">BT xem tranh</a> |<?php } ?>
                <?php if(acceptpermiss("luyennguam_update")) { ?><a href="<?php echo cpagerparm("tact,status,id,p") ?>tact=luyennguam_update&id=<?php echo $item['_id'] ?>">Sửa</a> |<?php }?>
                <?php if(acceptpermiss("luyennguam_delete")) { ?><a onclick="return confirm('Bạn chắc chắn chứ?')" href="<?php echo cpagerparm("tact,status,id") ?>tact=luyennguam_delete&id=<?php echo $item['_id'] ?>">Xóa</a><?php }?>
            </td>
        </tr>
    <?php } ?>
    </tbody>
    ...
    <script>
        function changeStatus (atid, status) {
            ...
            $.post('incoming.php',{act: 'changestatuslna', atid: atid, status: status, isUpdateTime: isUpdateTime}, function(res) {
                if (res.status == 200) {
        ...
        function changeFree (atid, status) {
            $.post('incoming.php',{act: 'changefree', atid: atid, status: status, type: '<?php echo Constant::TYPE_LUYENNGUAM ?>'}, function(res) {
                if (res.status == 200) {
