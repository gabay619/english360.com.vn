<?php
$title = "Quản lý bài tập trắc nghiêm - Giao tiếp cơ bản";
...
$cond['gtcbid'] = $gtcbid;
$type = array('gtcbid'=>$gtcbid,'type'=>'gtcb_tracnghiem');
$sort = array("_id" => -1);
...
<?php if(acceptpermiss("gtcb_test_insert")) { ?><a href="<?php echo cpagerparm("tact,id,status") ?>tact=gtcb_test_insert">Thêm mới</a><?php }?>
...
<table class="table table-hover">
    ...
    <?php foreach ($listnews as $item) { ?>
        <tr>
            ...
            <td>
                <?php if(acceptpermiss("gtcb_test_update")) { ?><a href="<?php echo cpagerparm("tact,status,id") ?>tact=gtcb_test_update&id=<?php echo $item['_id'] ?>">Sửa</a> |<?php }?>
                <?php if(acceptpermiss("gtcb_test_del")) { ?><a onclick="return confirm('Bạn chắc chắn chứ?')" href="<?php echo cpagerparm("tact,status,id") ?>tact=gtcb_test_del&id=<?php echo $item['_id'] ?>">Xóa</a><?php }?>
            </td>
        </tr>
    <?php } ?>
    </tbody>
