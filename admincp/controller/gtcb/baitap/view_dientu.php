...
$type = array('gtcbid'=>$gtcbid,'type'=>'gtcb_dientu');
$cond['gtcbid'] = $gtcbid;
$cond['type'] = $type;
$sort = array("_id" => -1);
    ...
    <table class="table table-hover">
        ...
        <?php foreach ($listnews as $item) { ?>
            <tr>
                ...
                <td>
                    <?php if(acceptpermiss("gtcb_listen_update")) { ?><a href="<?php echo cpagerparm("tact,status,id") ?>tact=gtcb_dt_update&id=<?php echo $item['_id'] ?>">Sửa</a> |<?php }?>
                    <?php if(acceptpermiss("gtcb_listen_del")) { ?><a onclick="return confirm('Bạn chắc chắn chứ?')" href="<?php echo cpagerparm("tact,status,id") ?>tact=gtcb_dt_del&id=<?php echo $item['_id'] ?>">Xóa</a><?php }?>
                </td>
            </tr>
        <?php } ?>
        </tbody>
