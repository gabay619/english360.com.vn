<?php
$title = "Thông tin bài tập điền từ theo tranh - ngữ pháp";
...
<div class="text-left"><a href="<?php echo cpagerparm("tact,id") ?>tact=np_dtt_view ">Trở về danh sách bài tập</a></div>
...
if (isset($_POST['acpt'])) {
    ...
    if ($tact == "np_dtt_insert") {
        $_POST['_id'] = (string)strtotime("now");
        $_POST['datecreate'] = time();
        $_POST['type'] = "nguphap_dientutranh";
...
##Get Data
if ($tact != "np_dtt_insert") $_POST = (array)$newscl->findOne(array("_id" => "$id"));
?>
<form class="form-horizontal" role="form" action="" method="post">
    <ul class="nav nav-tabs" role="tablist" id="myTab">
        <li class="active"><a href="#info" role="tab" data-toggle="tab">Thông tin bài tập điền từ theo tranh</a></li>
    </ul>
        ...
        <div class="tab-pane active" id="info">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    ...
                    <p class="help-block">Upload file ảnh và audio trên /fm rồi paste link vào ô</p>
                </div>
            </div>
            <div class="listaw">
                <?php
                foreach($_POST['question'] as $key=>$item){
                    ...
                    <div class="form-group" data-order="<?php echo $key?>">
                        ...
                        <div class="col-sm-3">
                            <input type="text" placeholder="Câu, bỏ trống bằng dấu _" name="question[<?php echo $key?>][sentence]" class="form-control" value="<?php echo $item['sentence'] ?>">
                        </div>
                        <div class="col-sm-2">
                            <input type="text" placeholder="Đáp án, cách nhau |" name="question[<?php echo $key?>][aw]" class="form-control" value="<?php echo $item['aw'] ?>">
                        </div>
                        <div class="col-sm-2">
                            <input type="text" placeholder="Link audio" name="question[<?php echo $key?>][audio]" class="form-control" value="<?php echo $item['audio'] ?>">
                        </div>
                        <div class="col-sm-2">
                            <input type="text" placeholder="Link ảnh" name="question[<?php echo $key?>][pic]" class="form-control" value="<?php echo $item['pic'] ?>">
                        </div>
                        <div class="col-sm-1">
    ...
    <div class="form-group">
        <div class="col-sm-offset-2 col-sm-10">
            ...
            hoặc <a href="<?php echo cpagerparm("tact,id") ?>tact=np_dtt_view">Thoát</a> |
...
<div>
    ...
    <img src="/admincp/asset/images/nguphap/nguphap_dientutranh.png" alt="" style="display: none; border: 1px dashed #ccc" >
</div>
<script>
    $(function () {
...
//        console.log(lastOrder);return false;
        html = '<div class="form-group" data-order="'+next+'">'+
            ...
            '</div>'+
            '<div class="col-sm-2">'+
            '<input type="text" placeholder="Đáp án, cách nhau |" name="question['+next+'][aw]" class="form-control">'+
            '</div>'+
            '<div class="col-sm-2">'+
            '<input type="text" placeholder="Link audio" name="question['+next+'][audio]" class="form-control">'+
            '</div>'+
            '<div class="col-sm-2">'+
            '<input type="text" placeholder="Link ảnh" name="question['+next+'][pic]" class="form-control">'+
            '</div>'+
            '<div class="col-sm-1">'+
            '<a href="javascript:void(0)" onclick="$(this).parent().parent().remove()">Xóa</a>'+
            '</div>'+
