...
//        $lastChat = $item['chat'][count($item['chat']) - 1];
        ...
        <tr>
            ...
            <td>
                <?php if($user['aff_status'] == Constant::STATUS_DISABLE): ?>
            ...
            <td>
                ...
                <?php if($user['account_balance'] >= Constant::WITHDRAW_MIN_PAY): ?>
                    <button type="button" class="btn btn-sm btn-info" onclick="withdraw('<?php echo $item['_id'] ?>',<?php echo $user['account_balance'] ?>)">Tạo lệnh rút tiền</button>
                <?php endif; ?>
                <?php if($user['aff_status'] == Constant::STATUS_DISABLE): ?>
                    <button type="button" class="btn btn-sm btn-success" onclick="unlock('<?php echo $user['_id'] ?>')"><i class="fa fa-unlock-alt"></i></button>
                <?php  else:  ?>
                    <button type="button" class="btn btn-sm btn-danger" onclick="lock('<?php echo $user['_id'] ?>')"><i class="fa fa-lock"></i></button>
                <?php endif; ?>
            </td>
        </tr>
    <?php endforeach; ?>
